...
# LLM 模型設定
SCREENING_MODEL = "gpt-5-mini"   # 初篩（全部摘要）
ANALYSIS_MODEL  = "gpt-5"        # 精讀（Medium / High 專利）
...
CUSTOM_QUERIES = [
    ...
    # ── 新增（同事要求）─────────────────────────────────────
    # IPF × mast cell stabilizer（用 IPF 縮寫重搜一次）
    'ta="mast cell stabilizer" AND ta="IPF"',
    # Pemirolast × lung fibrosis
    'ta=pemirolast AND ta="lung fibrosis"',
    'ta=pemirolast AND ta="pulmonary fibrosis"',
    # Pemirolast × combination therapy × lung fibrosis
    'ta=pemirolast AND ta="combination" AND ta="fibrosis"',
    'ta=pemirolast AND ta="combination" AND ta="pulmonary"',
    # Pemirolast × GDF-15 × lung fibrosis
    'ta=pemirolast AND ta="GDF-15"',
    'ta="GDF-15" AND ta="lung fibrosis"',
    'ta="GDF-15" AND ta="pulmonary fibrosis"',
    'ta="growth differentiation factor 15" AND ta="fibrosis"',
    # GDF-15 × mast cell（交叉機制）
    'ta="GDF-15" AND ta="mast cell"',
]